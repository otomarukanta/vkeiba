...
regex = dict()
regex['race_id'] = re.compile("race\/result\/(\d+)")
regex['path'] = re.compile("/\w+/\w+/(\d+)/")
regex['sex_age'] = re.compile("(\w+?)(\d+)")
regex['jockey_weight'] = re.compile("\w*?(\d+\.\d+)")
regex['weight'] = re.compile("(\d*)\((.*?)\)")
regex['day'] = re.compile(r"(\d*)年(\d*)月(\d*)日.*?(\d*)回"
                       "(.*?)(\d*)日.*?(\d*:\d*)発走")
regex['title'] = re.compile(r"競馬 - (.+?) 結果 - スポーツナビ")
regex['meta'] = re.compile(r"(.+?)・(.+?) (\d+?)m \[コースガイド\]"
                        " \| 天気： \| 馬場： \| (.+?) \| (.+?)"
                        " \| (.+?) \|")
...
def parse_race_result(soup):
    def parse_result():
        ...
        def trim_record(record, idx):
            ...
            res['horse_id'] = regex['path'].findall(record[3].a['href'])[0]
            res['sex'], res['age'] = regex['sex_age'].findall(record[4].text)[0]
            res['jockey_id'] = regex['path'].findall(record[5].a['href'])[0]
            res['jockey_weight'] = regex['jockey_weight'].findall(
                record[10].text)[0]
            if res['final_position'].strip() == '取消':
                return {k: v.strip() for k, v in res.items()}
            weight = regex['weight'].findall(record[11].text)[0][0]
            if weight:
    ...
    def parse_info():
        ...
        res['race_name'] = regex['title'].findall(soup.title.text)[0]
    ...
    if any(x in url for x in ['denma', 'regist']):
        return
    race_id = regex['race_id'].findall(url)[0]
